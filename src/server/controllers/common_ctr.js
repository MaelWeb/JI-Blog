...
export async function getOneContent(ctx) {
    ...
    await Superagent.get('http://wufazhuce.com').then(res => {
        if (!res) {
      result = {
                code: 500,
                message: err,
        ...
        let data = [];
        $('#carousel-one .item').each((idx, element) => {
...
export async function pushToBaidu(ctx) {
    ...
    await Superagent.post(
        `http://data.zz.baidu.com/urls?site=www.liayal.com&token=kdDIO6Gl2EqQOofj`,
    )
        .send(postData.url)
        .then(res => {
      result = res.body;
...
export async function geeTestRegister(ctx) {
    ...
    const gtConfig = await GeeTest.register({
        ...
        client_type:
      clientType.isTablet || clientType.isAndroid || clientType.isPhone
          ? 'h5'
          : "web"
    });
    ...
    if (!gtConfig) {
        ...
        ctx.session.fallback = gtConfig.code == 201;
        result.data = gtConfig.data;
        result.data.offline = gtConfig.code == 201;
    }
